...
def save_dataset_csv(args):
    ...
    if os.path.isfile(dataset_csv_path):
        ...
        print(f"警告: {dataset_csv_path} 是一个目录而不是文件，跳过复制操作")
...
def cal_is_stopping(args, epoch_info_log, process_pipeline):
    ...
    # 如果评价指标是loss，则将loss取反，转换为"越大越好"
    if judge_metric == 'val_loss':
        judge_metric_list = -np.array(judge_metric_list)
    last_epoch = epoch_info_log['epoch'][-1]
    ref_index = last_epoch - patience - 1  # 使用耐心前一个epoch作为参考
    ref_metric = judge_metric_list[ref_index]

    # 如果评价指标不是loss，同时获取对应的loss列表用于平局时比较
    if judge_metric != 'val_loss':
        loss_list = epoch_info_log['val_loss']
        ref_loss = loss_list[ref_index]

    # 检查在过去的patience个epoch中是否存在改善
    for i in range(last_epoch, last_epoch - patience, -1):
        cur_metric = judge_metric_list[i - 1]
        # 对于loss情况，只要指标有提升即可视为进步（因为已取负，越大越好）
        if cur_metric > ref_metric:
            return False
        # 对于非loss指标，如果指标提升或指标持平但loss有下降，则视为进步
        if judge_metric != 'val_loss' and cur_metric == ref_metric:
            cur_loss = loss_list[i - 1]
            # 若当前loss低于参考loss，说明模型在该轮次有所进步
...
def merge_k_fold_logs(k_fold_log_dir, process_pipeline):
    ...
    # 打印各个指标的均值，保留小数点后三位
    print("各指标均值（保留三位小数）:")
    for key, value in k_fold_metrics_mean_std.items():
