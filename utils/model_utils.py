...
def get_optimizer(args,model):
    ...
    if opt == 'adam':
       ...
       lr = args.Model.optimizer.sgd_config.lr
       momentum = args.Model.optimizer.sgd_config.momentum
       weight_decay = args.Model.optimizer.sgd_config.weight_decay
       optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=lr, momentum=momentum, weight_decay=weight_decay)
       return optimizer,lr
...
def get_model_from_yaml(yaml_args):
    ...
    if model_name == 'AB_MIL':
        ...
        instance_loss_fn = get_criterion(yaml_args.Model.instance_loss_fn)
        mil_model = CLAM_MB_MIL_Fusion(yaml_args.Model.gate,yaml_args.Model.size_arg,yaml_args.Model.dropout,yaml_args.Model.k_sample,yaml_args.General.num_classes,
                                instance_loss_fn,yaml_args.Model.subtyping,yaml_args.Model.embed_dim,yaml_args.Model.act,yaml_args.Model.instance_eval,
                                in_dims = yaml_args.Model.in_dims)
        return mil_model
