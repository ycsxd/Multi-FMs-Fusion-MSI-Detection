...
def compute_w_loader(args,file_path, output_path, wsi, model,
	...
	for count, (batch, coords) in enumerate(loader):
		...
		with torch.no_grad():	
			...
			# print(batch.shape)
			# print("Before passing to model: dtype=", batch.dtype, "mean=", batch.mean().item(), "std=", batch.std().item()) 
...
def main(args):
	...
	for bag_candidate_idx in range(total):
		...
		custom_downsample=custom_downsample, target_patch_size=args.target_patch_size)
			...
			print(f'{slide_file_path} failed')
			continue
...
parser.add_argument('--backbone', default='',type=str,choices=['vit_s_imagenet','Hibou-L','H-optimus-0',
															   'resnet50_imagenet','musk','plip','conch','uni','uni2',
															   'ctranspath','gigapath','virchow','virchow_v2',
															   'conch_v1_5'], help='backbone model')
