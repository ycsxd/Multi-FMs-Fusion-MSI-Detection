...
def parse_arguments():
    ...
    parser.add_argument('--label_txt', type=str, default='C:/Users/guoweis/Desktop/MIL_BASELINE-main/testdata/label2.txt',
                        help='标签 TXT 文件路径。')
    parser.add_argument('--input_folder', type=str, default='I:/GasMSI/gastric/svs/',
                        help='包含图像文件（TIFF 或 SVS）的输入文件夹。')
    parser.add_argument('--output_folder', type=str, default='I:/sgw/Features_zhe1/',
                        help='存储生成特征矩阵的输出文件夹。')
...
def load_and_parallel_model(model_choice):
    ...
    if model_choice == 'PathoDuetVits':
        ...
        model.load_state_dict(torch.load("I:/sgw/checkpoint/virchow/pytorch_model.bin", weights_only=True), strict=True)
...
def process_file(args):
    ...
    (base_name, labels, input_folder, output_folder, THRESHOLD, PATCH_SIZE,
     EDGE_THRESHOLD, model_choice, target_mpp, file_format) = args
    ...
    try:
        ...
        if file_format == 'tif':
            ...
            if target_mpp == '1.0':
                ...
                print(f"将图像 {base_name} 的 mpp 调整为 {target_mpp}, 放大倍数为 10x")
            elif target_mpp == '0.5':
                new_width = img.shape[1] // 2
                new_height = img.shape[0] // 2
                print(f"将图像 {base_name} 的 mpp 调整为 {target_mpp}, 放大倍数为 20x")
            else:
                raise ValueError("未知的 mpp 选择")
            ...
            for i in range(0, len(patch_list), batch_size):
                ...
                with torch.no_grad():
                    with torch.cuda.amp.autocast():
                        batch_features = global_model(batch_tensor)
...
def main():
    ...
    for fl in filenames:
        args_tuple = (fl, labels, input_folder, output_folder, THRESHOLD,
                      PATCH_SIZE, EDGE_THRESHOLD, model_choice, mpp, file_format)
        args_list.append(args_tuple)

    with Pool(processes=1, initializer=init_worker, initargs=(model_choice,)) as pool:
