'''
Segment TIFF文件生成特征矩阵

python tiff2feat.py --model_choice uni2 --input_folder I:/sgw/zhe1_10xtif/4/ --output_folder I:/sgw/zhe1_10xtif/4/ > uni2_feat_zhe1.log
'''
import warnings
...
def parse_arguments():
    ...
    parser.add_argument('--label_txt', type=str, default='',
                        help='标签 TXT 文件路径。')
    parser.add_argument('--input_folder', type=str, default='I:/sgw/tif/',
                        help='包含图像文件（TIFF 或 SVS）的输入文件夹。')
    parser.add_argument('--output_folder', type=str, default='I:/sgw/MIL_BASELINE-main/testdata/shenyang/',
                        help='存储生成特征矩阵的输出文件夹。')
...
def load_and_parallel_model(model_choice):
    ...
    if model_choice == 'PathoDuetVits':
        ...
        model.load_state_dict(torch.load("I:/sgw/checkpoint/checkpoint_HE.pth", weights_only=True), strict=True)
    elif model_choice == 'gigapath':
        model = timm.create_model("hf_hub:prov-gigapath/prov-gigapath")
        model.load_state_dict(torch.load('I:/sgw/checkpoint/pytorch_model_giga.bin', weights_only=True), strict=True)
        ...
        model.load_state_dict(torch.load("I:/sgw/checkpoint/pytorch_model.bin", weights_only=True), strict=True)
    elif model_choice == 'ctranspath':
        from models.ctran import ctranspath
        model = ctranspath()  # ctranspath 需要安装 timm-0.5.4.tar 包
        model.load_state_dict(torch.load("I:/sgw/checkpoint/ctranspath.pth", weights_only=True)['model'], strict=True)
    elif model_choice == 'virchow2':
        from timm.layers import SwiGLUPacked
        model = timm.create_model("hf-hub:paige-ai/Virchow2", pretrained=False,
                                  mlp_layer=SwiGLUPacked, act_layer=torch.nn.SiLU)
        model.load_state_dict(torch.load("I:/sgw/checkpoint/virchow2/pytorch_model.bin", weights_only=True), strict=True)
        ...
        model.load_state_dict(torch.load("I:/sgw/checkpoint/uni2/pytorch_model.bin", weights_only=True), strict=True)
    elif model_choice == 'musk':
        from musk import utils, modeling
        model = timm.create_model("musk_large_patch16_384")
        utils.load_model_and_may_interpolate("I:/sgw/checkpoint/musk/model.safetensors", model, 'model|module', 'weights_only=True')
    else:
...
def feature_extractor_adapter(model, batch,model_name):
	if model_name == 'plip':
		...
		features = model(batch)
	return features
def init_worker(model_choice):
    """
    每个 worker 进程启动时调用，加载一次模型。
    """
    global global_model
    global_model = load_and_parallel_model(model_choice)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    global_model.to(device)
    print(f"\nWorker {os.getpid()} 已加载模型: {model_choice}")

def process_file(args):
    """
    处理单个文件（TIFF ）：按照阈值和边缘检测条件分割图像为图块，
    然后通过预训练模型提取特征，最终保存特征矩阵及返回对应标签。
    """
    global global_model
    (base_name, labels, input_folder, output_folder, THRESHOLD, PATCH_SIZE,
     EDGE_THRESHOLD, model_choice) = args
    ...
    if global_model is None:
        raise ValueError("模型未初始化，请检查 worker 的初始化部分。")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    expected_filename = f'{base_name}.tif'
    ...
    try:
        ...
        for y in range(0, height_img - PATCH_SIZE + 1, step_size):
            for x in range(0, width_img - PATCH_SIZE + 1, step_size):
                # 从预计算的二值图像和边缘图像中截取对应区域
                binary_patch = binary_img[y:y + PATCH_SIZE, x:x + PATCH_SIZE]
                foreground_ratio = np.mean(binary_patch == 0)
                if foreground_ratio > 0.5:
                    patch_edges = edges_img[y:y + PATCH_SIZE, x:x + PATCH_SIZE]
                    edge_count = np.sum(patch_edges > 0)
                    if edge_count > EDGE_THRESHOLD:
                        patch = img[y:y + PATCH_SIZE, x:x + PATCH_SIZE]
                        patch_tensor = transform(patch).to(device)
        ...
        if patch_list:
            ...
            for i in range(0, len(patch_list), batch_size):
                batch_tensor = torch.stack(patch_list[i:i + batch_size])
                with torch.no_grad():
                    with torch.cuda.amp.autocast():
                        batch_features = feature_extractor_adapter(global_model, batch_tensor,model_choice)
        ...
        print(f"处理 {base_name} 时出现未知错误: {e}")
    return None
...
def main():
    ...
    if os.path.exists(label_txt):
        ...
        print(f"未找到标签文件 {label_txt}，直接遍历处理输入文件夹中的所有 tif 文件")
        for file in os.listdir(input_folder):
            if file.lower().endswith('.tif'):
                base_name = file[:-4]  # 去除 .tif 后缀
                filenames.append(base_name)
                labels[base_name] = "None"
    ...
    for fl in filenames:
        args_tuple = (fl, labels, input_folder, output_folder, THRESHOLD,
                      PATCH_SIZE, EDGE_THRESHOLD, model_choice)
    ...
    if not os.path.exists(csv_path):
        with open(csv_path, 'w') as f:
            f.write("test_slide_path,test_label\n")

    with Pool(processes=4, initializer=init_worker, initargs=(model_choice,)) as pool:
