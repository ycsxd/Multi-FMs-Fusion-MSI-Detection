...
from modules.CLAM_MB_MIL.clam_mb_mil_ensemble import CLAM_MB_MIL_ENSEMBLE
...
def ensemble_train_loop(device, model, loader, criterion, optimizer, scheduler, bag_weight):
    """
    ensemble_train_loop 用于训练 ensemble 模型。
...
def ensemble_val_loop(device, num_classes, model, loader, criterion, bag_weight,
                      return_WSI_feature=False, return_WSI_attn=False):
    """
    ensemble_val_loop 用于验证 ensemble 模型。
...
def process_CLAM_MB_MIL_ENSEMBLE(args):
    ...
    mil_model = CLAM_MB_MIL_ENSEMBLE(
    ...
    for epoch in tqdm(range(args.General.num_epochs),colour='GREEN'):
        ...
        train_loss,cost_time = ensemble_train_loop(device,mil_model,train_dataloader,criterion,optimizer,now_scheduler,bag_weight)
        if process_pipeline == 'Train_Val_Test':
            val_loss,val_metrics = ensemble_val_loop(device,num_classes,mil_model,val_dataloader,criterion,bag_weight)
